...
class ISCSIPortalIModel(sa.Model):
    ...
    __table_args__ = (
        Index('services_iscsitargetportalip_iscsi_target_portalip_ip__iscsi_target_portalip_port',
...
class iSCSITargetToExtentModel(sa.Model):
    ...
    __table_args__ = (
        Index('services_iscsitargettoextent_iscsi_target_id_757cc851_uniq',
              'iscsi_target_id', 'iscsi_extent_id',
              unique=True),
    )

    id = sa.Column(sa.Integer(), primary_key=True)
    iscsi_extent_id = sa.Column(sa.ForeignKey('services_iscsitargetextent__old.id'), index=True)
